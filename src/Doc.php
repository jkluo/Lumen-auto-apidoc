...
class Doc
{
    ...
    public function getInfo($class, $action)
    {
        ...
        if($class && class_exists($class)){
            ...
            if($reflection->hasMethod($action)) {
                ...
                $uses = $class."@".$method->name;
                $currentRoutes = $this->getCurrentRoute($uses);
                $action_doc['method'] = $currentRoutes['method'];
                $action_doc['url'] = $currentRoutes['url'];
            }
        }
        return $action_doc;
    }
                    ...
                    if(!in_array($action->name, $filter_method) && $action->class == $class)
                    {
                        ...
                        if($doc_str)
                        {
                            ...
                            foreach ($all_routes as $route){
                                if (!empty($route['action']['uses']) && $route['action']['uses'] == $class."@".$action->name){
                                    $action_doc['url'] = $route['uri'];
                                    $action_doc['method'] = $route['method'];
                                }
                            }
     ...
     * @param $uses
     */
    private function getCurrentRoute($uses){
        $all_routes=Route::getRoutes()??[];
        $method = '';
        $uri = '';
        foreach ($all_routes as $key => $value){
            if (isset($value['action']['uses']) && $value['action']['uses'] == $uses){
                $method = $value['method'];
                $uri = $value['uri'];
                break;
            }
        }
        return ['method'=>$method,'url'=>$uri];
    }
}